...
void Test_isCurrentOrderOfSummands::testIsCurrentOrderOfSummands_data()
{
    ...
    OperandOfExpr elem_1_2_1 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    OperandOfExpr elem_1_2_2 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    QTest::newRow("1.2 Correct order: the first one is a summand with the same variable names inside it. The current order is uncorrect") << elem_1_2_1 << elem_1_2_2 << false;
    ...
    OperandOfExpr elem_1_3_2 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    QTest::newRow("1.3 Correct order: the first one is a summand with the same variable names inside it.\nThe first term has an expression in parentheses, the order is correct") << elem_1_3_1 << elem_1_3_2 << true;
    ...
    OperandOfExpr elem_1_4_2 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    QTest::newRow("1.4 Correct order: the first one is a summand with the same variable names inside it.\nThe first term has an expression in parentheses, the order is uncorrect") << elem_1_4_1 << elem_1_4_2 << false;

    // 1.5 Второе слагаемое имеет выражение в скобках, порядок верный
    OperandOfExpr elem_1_5_1 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    OperandOfExpr elem_1_5_2 = OperandOfExpr{parent_plus, postfixToTree("y z 1 + *", errors), false};
    QTest::newRow("1.5 Correct order: the first one is a summand with the same variable names inside it.\nThe second term has an expression in parentheses, the order is correct") << elem_1_5_1 << elem_1_5_2 << true;

    // 1.6 Первый узел есть выражение в скобках, второй - число
    OperandOfExpr elem_1_6_1 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    OperandOfExpr elem_1_6_2 = OperandOfExpr{parent_plus, postfixToTree("x x 1 + *", errors), false};
    QTest::newRow("1.6 Correct order: the first one is a summand with the same variable names inside it.\nThe second term has an expression in parentheses, the order is uncorrect") << elem_1_6_1 << elem_1_6_2 << false;

    // 1.7 Оба слагаемых имеют выражение в скобках, порядок верный
    OperandOfExpr elem_1_7_1 = OperandOfExpr{parent_plus, postfixToTree("x x 1 + *", errors), false};
    OperandOfExpr elem_1_7_2 = OperandOfExpr{parent_plus, postfixToTree("y z 1 + *", errors), false};
    QTest::newRow("1.7 Correct order: the first one is a summand with the same variable names inside it.\nBoth term have expression in parentheses, the order is correct") << elem_1_7_1 << elem_1_7_2 << true;

    // 1.8 Оба слагаемых имеют выражение в скобках, порядок неверный
    OperandOfExpr elem_1_8_1 = OperandOfExpr{parent_plus, postfixToTree("y z 1 + *", errors), false};
    OperandOfExpr elem_1_8_2 = OperandOfExpr{parent_plus, postfixToTree("x x 1 + *", errors), false};
    QTest::newRow("1.8 Correct order: the first one is a summand with the same variable names inside it.\nBoth term have expression in parentheses, the order is incorrect") << elem_1_8_1 << elem_1_8_2 << false;

    // 1.9 Различная степень переменных, порядок верный
    OperandOfExpr elem_1_9_1 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    OperandOfExpr elem_1_9_2 = OperandOfExpr{parent_plus, postfixToTree("x y * z *", errors), false};
    QTest::newRow("1.9 Correct order: the first one is a summand with the same variable names inside it.\nDifferent variables' degrees, the order is correct") << elem_1_9_1 << elem_1_9_2 << true;

    // 1.10 Различная степень переменных, порядок неверный
    OperandOfExpr elem_1_10_1 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    OperandOfExpr elem_1_10_2 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    QTest::newRow("1.10 Correct order: the first one is a summand with the same variable names inside it.\nDifferent variables' degrees, the order is incorrect") << elem_1_10_1 << elem_1_10_2 << false;
    ...
    OperandOfExpr elem_1_12_1 = OperandOfExpr{parent_plus, postfixToTree("x x * x y + * x +", errors), false};
    OperandOfExpr elem_1_12_2 = OperandOfExpr{parent_plus, postfixToTree("x x * x x + * x +", errors), false};
    QTest::newRow("1.12 Correct order: the first one is a summand with the same variable names inside it.\nMany nested parentheses, the order is incorrect") << elem_1_12_1 << elem_1_12_2 << false;

    // 1.13 Равные слагаемые
    OperandOfExpr elem_1_13_1 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    QTest::newRow("1.13 Correct order: the first one is a summand with the same variable names inside it.\nEqual terms, the order is correct") << elem_1_13_1 << elem_1_13_1 << true;
    ...
    OperandOfExpr elem_2_2_1 = OperandOfExpr{parent_plus, postfixToTree("x y *", errors), false};
    OperandOfExpr elem_2_2_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("2.2 Correct order: first comes the term with a higher degree of variables.\nThere are more variables in the second term than in the first") << elem_2_2_1 << elem_2_2_2 << false;

    // 2.3 Второе слагаемое без переменных
    OperandOfExpr elem_2_3_1 = OperandOfExpr{parent_plus, postfixToTree("x y *", errors), false};
    OperandOfExpr elem_2_3_2 = OperandOfExpr{parent_plus, postfixToTree("1", errors), false};
    QTest::newRow("2.3 Correct order: first comes the term with a higher degree of variables.\nSecond term without variables") << elem_2_3_1 << elem_2_3_2 << true;

    // 2.4 Первое слагаемое  без переменных
    OperandOfExpr elem_2_4_1 = OperandOfExpr{parent_plus, postfixToTree("1", errors), false};
    OperandOfExpr elem_2_4_2 = OperandOfExpr{parent_plus, postfixToTree("x y *", errors), false};
    QTest::newRow("2.4 Correct order: first comes the term with a higher degree of variables.\nFirst term without variables") << elem_2_4_1 << elem_2_4_2 << false;
    ...
    OperandOfExpr elem_2_5_2 = OperandOfExpr{parent_plus, postfixToTree("x y *", errors), false};
    QTest::newRow("2.5 Correct order: first comes the term with a higher degree of variables.\nFirst term has expression in parentheses, the order is correct") << elem_2_5_1 << elem_2_5_2 << true;
    ...
    OperandOfExpr elem_2_7_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    OperandOfExpr elem_2_7_2 = OperandOfExpr{parent_plus, postfixToTree("x y 1 + *", errors), false};
    QTest::newRow("2.7 Correct order: first comes the term with a higher degree of variables.\nSecond term has expression in parentheses, the order is correct") << elem_2_7_1 << elem_2_7_2 << true;

    // 2.8 Второе слагаемое имеет выражение в скобках, порядок неверный
    OperandOfExpr elem_2_8_1 = OperandOfExpr{parent_plus, postfixToTree("x y *", errors), false};
    OperandOfExpr elem_2_8_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * 1 + *", errors), false};
    QTest::newRow("2.8 Correct order: first comes the term with a higher degree of variables.\nSecond term has expression in parentheses, the order is incorrect") << elem_2_8_1 << elem_2_8_2 << false;

    // 2.9 Оба слагаемых имеют выражение в скобках, порядок верный
    OperandOfExpr elem_2_9_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * 1 + *", errors), false};
    OperandOfExpr elem_2_9_2 = OperandOfExpr{parent_plus, postfixToTree("x y 1 + *", errors), false};
    QTest::newRow("2.9 Correct order: first comes the term with a higher degree of variables.\nBoth terms have expression in parentheses, the order is correct") << elem_2_9_1 << elem_2_9_2 << true;

    // 2.10 Оба слагаемых имеют выражение в скобках, порядок неверный
    OperandOfExpr elem_2_10_1 = OperandOfExpr{parent_plus, postfixToTree("x y 1 + *", errors), false};
    OperandOfExpr elem_2_10_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * 1 + *", errors), false};
    QTest::newRow("2.10 Correct order: first comes the term with a higher degree of variables.\nBoth terms have expression in parentheses, the order is incorrect") << elem_2_10_1 << elem_2_10_2 << false;
    ...
    OperandOfExpr elem_2_12_1 = OperandOfExpr{parent_plus, postfixToTree("x y 1 + * 3 2 + *", errors), false};
    OperandOfExpr elem_2_12_2 = OperandOfExpr{parent_plus, postfixToTree("a b 1 + * c 2 + *", errors), false};
    QTest::newRow("2.12 Correct order: first comes the term with a higher degree of variables.\nSeveral terms, the order is incorrect") << elem_2_12_1 << elem_2_12_2 << false;
    ...
    OperandOfExpr elem_2_14_1 = OperandOfExpr{parent_plus, postfixToTree("a b 1 + *", errors), false};
    OperandOfExpr elem_2_14_2 = OperandOfExpr{parent_plus, postfixToTree("x 1 y z 2 + * + *", errors), false};
    QTest::newRow("2.14 Correct order: first comes the term with a higher degree of variables.\nNested expressions in parentheses, the order is incorrect") << elem_2_14_1 << elem_2_14_2 << false;
    ...
    OperandOfExpr elem_2_16_1 = OperandOfExpr{parent_plus, postfixToTree("y y *", errors), false};
    OperandOfExpr elem_2_16_2 = OperandOfExpr{parent_plus, postfixToTree("x x x * *", errors), false};
    QTest::newRow("2.16 Correct order: first comes the term with a higher degree of variables.\nVariables' names are equal, the order is incorrect") << elem_2_16_1 << elem_2_16_2 << false;

    // 3.Проверка на правило: по алфавиту, учитывая только имена переменные

    // 3.1 Отличны первые переменные, порядок верный
    OperandOfExpr elem_3_1_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    OperandOfExpr elem_3_1_2 = OperandOfExpr{parent_plus, postfixToTree("x b c * *", errors), false};
    QTest::newRow("3.1 Correct order: alphabetically, considering only the names of the variables.\nThe first variables are excellent, the order is correct") << elem_3_1_1 << elem_3_1_2 << true;

    // 3.2 Отличны первые переменные, порядок неверный
    OperandOfExpr elem_3_2_1 = OperandOfExpr{parent_plus, postfixToTree("x b c * *", errors), false};
    OperandOfExpr elem_3_2_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("3.2 Correct order: alphabetically, considering only the names of the variables.\nThe first variables are excellent, the order is uncorrect") << elem_3_2_1 << elem_3_2_2 << false;

    // 3.3 Отличны вторые переменные, порядок верный
    OperandOfExpr elem_3_3_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    OperandOfExpr elem_3_3_2 = OperandOfExpr{parent_plus, postfixToTree("a x c * *", errors), false};
    QTest::newRow("3.3 Correct order: alphabetically, considering only the names of the variables.\nThe second variables are different, the order is correct") << elem_3_3_1 << elem_3_3_2 << true;

    // 3.4 Отличны вторые переменные, порядок неверный
    OperandOfExpr elem_3_4_1 = OperandOfExpr{parent_plus, postfixToTree("a x c * *", errors), false};
    OperandOfExpr elem_3_4_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("3.4 Correct order: alphabetically, considering only the names of the variables.\nThe second variables are different, the order is incorrect") << elem_3_4_1 << elem_3_4_2 << false;

    // 3.5 Отличны последние переменные, порядок верный
    OperandOfExpr elem_3_5_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    OperandOfExpr elem_3_5_2 = OperandOfExpr{parent_plus, postfixToTree("a b * x *", errors), false};
    QTest::newRow("3.5 Correct order: alphabetically, considering only the names of the variables.\nThe last variables are different, the order is correct") << elem_3_5_1 << elem_3_5_2 << true;

    // 3.6 Отличны последние переменные, порядок неверный
    OperandOfExpr elem_3_6_1 = OperandOfExpr{parent_plus, postfixToTree("a b * x *", errors), false};
    OperandOfExpr elem_3_6_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("3.6 Correct order: alphabetically, considering only the names of the variables.\nThe last variables are different, the order is incorrect") << elem_3_6_1 << elem_3_6_2 << false;
    ...
    OperandOfExpr elem_3_7_2 = OperandOfExpr{parent_plus, postfixToTree("a b * x *", errors), false};
    QTest::newRow("3.7 Correct order: alphabetically, considering only the names of the variables.\nFirst term has expression in parentheses, the order is correct") << elem_3_7_1 << elem_3_7_2 << true;
    ...
    OperandOfExpr elem_3_8_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("3.8 Correct order: alphabetically, considering only the names of the variables.\nFirst term has expression in parentheses, the order is incorrect") << elem_3_8_1 << elem_3_8_2 << false;

    // 3.9 Второе слагаемое имеет выражение в скобках, порядок верный
    OperandOfExpr elem_3_9_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    OperandOfExpr elem_3_9_2 = OperandOfExpr{parent_plus, postfixToTree("a b x * 1 + *", errors), false};
    QTest::newRow("3.9 Correct order: alphabetically, considering only the names of the variables.\nSecond term has expression in parentheses, the order is correct") << elem_3_9_1 << elem_3_9_2 << true;

    // 3.10 Второе слагаемое имеет выражение в скобках, порядок неверный
    OperandOfExpr elem_3_10_1 = OperandOfExpr{parent_plus, postfixToTree("a b * x *", errors), false};
    OperandOfExpr elem_3_10_2 = OperandOfExpr{parent_plus, postfixToTree("a b c * 1 + *", errors), false};
    QTest::newRow("3.10 Correct order: alphabetically, considering only the names of the variables.\nSecond term has expression in parentheses, the order is incorrect") << elem_3_10_1 << elem_3_10_2 << false;
    ...
    OperandOfExpr elem_3_11_2 = OperandOfExpr{parent_plus, postfixToTree("a b x + *", errors), false};
    QTest::newRow("3.11 Correct order: alphabetically, considering only the names of the variables.\nBoth terms have expression in parentheses, the order is correct") << elem_3_11_1 << elem_3_11_2 << true;

    // 3.12 Оба слагаемых имеют выражение в скобках, порядок неверный
    OperandOfExpr elem_3_12_1 = OperandOfExpr{parent_plus, postfixToTree("a b x + *", errors), false};
    OperandOfExpr elem_3_12_2 = OperandOfExpr{parent_plus, postfixToTree("a b c + *", errors), false};
    QTest::newRow("3.12 Correct order: alphabetically, considering only the names of the variables.\nBoth terms have expression in parentheses, the order is incorrect") << elem_3_12_1 << elem_3_12_2 << false;
    ...
    OperandOfExpr elem_3_14_1 = OperandOfExpr{parent_plus, postfixToTree("a b c x 1 + * + *", errors), false};
    OperandOfExpr elem_3_14_2 = OperandOfExpr{parent_plus, postfixToTree("a b c d 1 + * + *", errors), false};
    QTest::newRow("3.14 Correct order: alphabetically, considering only the names of the variables.\nNested expressions in parentheses, the order is incorrect") << elem_3_14_1 << elem_3_14_2 << false;

    // 3.15 Равные слагаемые
    OperandOfExpr elem_3_15_1 = OperandOfExpr{parent_plus, postfixToTree("a b c * *", errors), false};
    QTest::newRow("3.15 Correct order: alphabetically, considering only the names of the variables.\nEqual terms, the order is correct") << elem_3_15_1 << elem_3_15_1 << true;
    ...
    OperandOfExpr expr_4_2_1 = OperandOfExpr{parent_plus, postfixToTree("4 a b * *", errors), false};
    OperandOfExpr expr_4_2_2 = OperandOfExpr{parent_plus, postfixToTree("2 3 * a b * *", errors), false};
    QTest::newRow("4.2 Correct order: from more elements of the summand to fewer.\nThere are more elements in the second term than in the first") << expr_4_2_1 << expr_4_2_2 << false;
    ...
    OperandOfExpr expr_4_3_2 = OperandOfExpr{parent_plus, postfixToTree("4 a b * *", errors), false};
    QTest::newRow("4.3 Correct order: from more elements of the summand to fewer.\nFirst term has expression in parentheses, the order is correct") << expr_4_3_1 << expr_4_3_2 << true;
    ...
    OperandOfExpr expr_4_4_2 = OperandOfExpr{parent_plus, postfixToTree("2 3 * a b * *", errors), false};
    QTest::newRow("4.4 Correct order: from more elements of the summand to fewer.\nFirst term has expression in parentheses, the order is incorrect") << expr_4_4_1 << expr_4_4_2 << false;

    // 4.5 Второе слагаемое имеет выражение в скобках, порядок верный
    OperandOfExpr expr_4_5_1 = OperandOfExpr{parent_plus, postfixToTree("2 3 * a b * *", errors), false};
    OperandOfExpr expr_4_5_2 = OperandOfExpr{parent_plus, postfixToTree("4 a b + *", errors), false};
    QTest::newRow("4.5 Correct order: from more elements of the summand to fewer.\nSecond term has expression in parentheses, the order is correct") << expr_4_5_1 << expr_4_5_2 << true;

    // 4.6 Второе слагаемое имеет выражение в скобках, порядок неверный
    OperandOfExpr expr_4_6_1 = OperandOfExpr{parent_plus, postfixToTree("4 a b * *", errors), false};
    OperandOfExpr expr_4_6_2 = OperandOfExpr{parent_plus, postfixToTree("2 a b * 3 + *", errors), false};
    QTest::newRow("4.6 Correct order: from more elements of the summand to fewer.\nSecond term has expression in parentheses, the order is incorrect") << expr_4_6_1 << expr_4_6_2 << false;

    // 4.7 Оба слагаемых имеют выражение в скобках, порядок верный
    OperandOfExpr expr_4_7_1 = OperandOfExpr{parent_plus, postfixToTree("2 a b * 3 + *", errors), false};
    OperandOfExpr expr_4_7_2 = OperandOfExpr{parent_plus, postfixToTree("4 a b + *", errors), false};
    QTest::newRow("4.7 Correct order: from more elements of the summand to fewer.\nBoth terms have expression in parentheses, the order is correct") << expr_4_7_1 << expr_4_7_2 << true;

    // 4.8 Оба слагаемых имеют выражение в скобках, порядок неверный
    OperandOfExpr expr_4_8_1 = OperandOfExpr{parent_plus, postfixToTree("4 a b + *", errors), false};
    OperandOfExpr expr_4_8_2 = OperandOfExpr{parent_plus, postfixToTree("2 a b * 3 + *", errors), false};
    QTest::newRow("4.8 Correct order: from more elements of the summand to fewer.\nBoth terms have expression in parentheses, the order is incorrect") << expr_4_8_1 << expr_4_8_2 << false;
    ...
    OperandOfExpr expr_4_9_2 = OperandOfExpr{parent_plus, postfixToTree("2 a b * 3 + *", errors), false};
    QTest::newRow("4.9 Correct order: from more elements of the summand to fewer.\nNested expressions in parentheses, the order is correct") << expr_4_9_1 << expr_4_9_2 << true;

    // 4.10 Вложенные выражения в скобках, порядок неверный
    OperandOfExpr expr_4_10_1 = OperandOfExpr{parent_plus, postfixToTree("2 a b * 3 + *", errors), false};
    OperandOfExpr expr_4_10_2 = OperandOfExpr{parent_plus, postfixToTree("4 a b 2 + * 1 + *", errors), false};
    QTest::newRow("4.10 Correct order: from more elements of the summand to fewer.\nNested expressions in parentheses, the order is incorrect") << expr_4_10_1 << expr_4_10_2 << false;
    ...
    OperandOfExpr expr_5_2_1 = OperandOfExpr{parent_plus, postfixToTree("a b c + 2 1 + + *", errors), false};
    OperandOfExpr expr_5_2_2 = OperandOfExpr{parent_plus, postfixToTree("a b c + * 2 1 + *", errors), false};
    QTest::newRow("5.2 Correct order: from more expressions in parentheses in the summand to fewer.\nThere are more expressions in parentheses in the second term than in the first one.") << expr_5_2_1 << expr_5_2_2 << false;
    ...
    OperandOfExpr expr_5_4_1 = OperandOfExpr{parent_plus, postfixToTree("b c + 2 1 + + d 3 + *", errors), false};
    OperandOfExpr expr_5_4_2 = OperandOfExpr{parent_plus, postfixToTree("b c + 2 1 + * d 3 + *", errors), false};
    QTest::newRow("5.4 Correct order: from more expressions in parentheses in the summand to fewer.\nTerms are exist of parentheses only, the order is incorrect") << expr_5_4_1 << expr_5_4_2 << false;
    ...
    OperandOfExpr expr_5_6_1 = OperandOfExpr{parent_plus, postfixToTree("b c 2 1 + * +", errors), false};
    OperandOfExpr expr_5_6_2 = OperandOfExpr{parent_plus, postfixToTree("b c + 2 1 + *", errors), false};
    QTest::newRow("5.6 Correct order: from more expressions in parentheses in the summand to fewer.\nNested expressions in parentheses, the order is incorrect") << expr_5_6_1 << expr_5_6_2 << false;
    ...
    OperandOfExpr expr_5_8_1 = OperandOfExpr{parent_plus, postfixToTree("1 a b - *", errors), false};
    OperandOfExpr expr_5_8_2 = OperandOfExpr{parent_plus, postfixToTree("1 a b + *", errors), false};
    QTest::newRow("5.8 Correct order: from more expressions in parentheses in the summand to fewer.\nEqual terms, different operators: «-», «+»") << expr_5_8_1 << expr_5_8_2 << true;
    ...
    OperandOfExpr expr_7_1_2 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    QTest::newRow("7.1 Unary cons that do not affect the order.\nThe first term with a unary minus, the order is correct") << expr_7_1_1 << expr_7_1_2 << true;
    ...
    OperandOfExpr expr_7_2_2 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    QTest::newRow("7.2 Unary cons that do not affect the order.\nThe first term with a unary minus, the order is uncorrect") << expr_7_2_1 << expr_7_2_2 << false;

    // 7.3 Второе слагаемое с унарным минусом, порядок верный
    OperandOfExpr expr_7_3_1 = OperandOfExpr{parent_plus, postfixToTree("x x *", errors), false};
    OperandOfExpr expr_7_3_2 = OperandOfExpr{parent_plus, postfixToTree("y z ~ *", errors), false};
    QTest::newRow("7.3 Unary cons that do not affect the order.\nSecond term with unary minus, the order is correct") << expr_7_3_1 << expr_7_3_2 << true;

    // 7.4 Второе слагаемое с унарным минусом, порядок неверный
    OperandOfExpr expr_7_4_1 = OperandOfExpr{parent_plus, postfixToTree("y z *", errors), false};
    OperandOfExpr expr_7_4_2 = OperandOfExpr{parent_plus, postfixToTree("x x ~ *", errors), false};
    QTest::newRow("7.4 Unary cons that do not affect the order.\nSecond term with unary minus, the order is incorrect") << expr_7_4_1 << expr_7_4_2 << false;
    ...
    OperandOfExpr expr_7_6_1 = OperandOfExpr{parent_plus, postfixToTree("y ~ ~ z ~ ~ *", errors), false};
    OperandOfExpr expr_7_6_2 = OperandOfExpr{parent_plus, postfixToTree("x ~ ~ x ~ ~ *", errors), false};
    QTest::newRow("7.6 Unary cons that do not affect the order.\nSeveral unary minuses, the order is incorrect") << expr_7_6_1 << expr_7_6_2 << false;
    ...
    OperandOfExpr expr_7_8_1 = OperandOfExpr{parent_plus, postfixToTree("y z + ~", errors), false};
    OperandOfExpr expr_7_8_2 = OperandOfExpr{parent_plus, postfixToTree("x x + ~", errors), false};
    QTest::newRow("7.8 Unary cons that do not affect the order.\nBoth terms are expressions in parentheses, the order is incorrect") << expr_7_8_1 << expr_7_8_2 << false;
    ...
    OperandOfExpr expr_7_10_1 = OperandOfExpr{parent_plus, postfixToTree("4 a b ~ + *", errors), false};
    OperandOfExpr expr_7_10_2 = OperandOfExpr{parent_plus, postfixToTree("2 a b ~ * 3 + *", errors), false};
    QTest::newRow("7.10 Unary cons that do not affect the order.\nUnary minus in expressions in parentheses, the order is incorrect") << expr_7_10_1 << expr_7_10_2 << false;
}
