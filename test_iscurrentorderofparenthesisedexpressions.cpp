...
void Test_isCurrentOrderOfParenthesisedExpressions::testIsCurrentOrderOfParenthesisedExpressions_data()
{
    ...
    QTest::newRow("1.2 Correct order: from a higher degree of the nth term variable to a lower one.\nWhen checking the first pair of terms, the incorrect order was determined.") << elem_1_1_2 << elem_1_1_1 << false;
    ...
    QTest::newRow("1.4 Correct order: from a higher degree of the nth term variable to a lower one.\nWhen checking the average pair of terms, the incorrect order was determined.") << elem_1_3_2 << elem_1_3_1 << false;
    ...
    OperandOfExpr elem_2_3_1 = OperandOfExpr{parent, postfixToTree("a x + c +", errors), false};
    QTest::newRow("2.3 Correct order: alphabetically, considering only variables.\nThe second variables are difficult, the order is correct") << elem_2_1_1 << elem_2_3_1 << true;
    ...
    QTest::newRow("2.4 Correct order: alphabetically, considering only variables.\nThe second variables are difficult, the order is incorrect") << elem_2_4_1 << elem_2_2_2 << false;

    // 2.5 Отличны последние переменные, порядок верный
    OperandOfExpr elem_2_5_1 = OperandOfExpr{parent, postfixToTree("a b + x +", errors), false};
    QTest::newRow("2.5 Correct order: alphabetically, considering only variables.\nThe last variables are difficult, the order is correct") << elem_2_1_1 << elem_2_5_1 << true;
    ...
    QTest::newRow("2.6 Correct order: alphabetically, considering only variables.\nThe last variables are difficult, the order is incorrect") << elem_2_6_1 << elem_2_2_2 << false;
    ...
    QTest::newRow("2.10 Correct order: alphabetically, considering only variables.\nNested expressions in parentheses, the order is incorrect") << elem_2_9_2 << elem_2_9_1 << false;
    ...
    QTest::newRow("3.2 Correct order: from more variables in an expression to fewer.\nThere are more variables in the second expression than in the first") << elem_3_1_2 << elem_3_1_1 << false;
    ...
    QTest::newRow("3.4 Correct order: from more variables in an expression to fewer.\nBoth expressions have expression in parentheses, the order is incorrect") << elem_3_3_2 << elem_3_3_1 << false;
    ...
    QTest::newRow("3.6 Correct order: from more variables in an expression to fewer.\nExpressions consist of multipliers, the order is incorrect") << elem_3_5_2 << elem_3_5_1 << false;
    ...
    QTest::newRow("3.8 Correct order: from more variables in an expression to fewer.\nNested expressions in parentheses, the order is incorrect") << elem_3_7_2 << elem_3_7_1 << false;
    ...
    QTest::newRow("4.3 Correct order: from more terms to fewer.\nFirst expression has expression in parentheses, the order is correct") << expr_4_3_1 << expr_4_1_2 << true;
    ...
    QTest::newRow("5.4 Unary cons that do not affect the order.\nMultiplier has unary minus, the order is incorrect") << expr_5_3_2 << expr_5_3_1 << false;
    ...
    QTest::newRow("6.2 Equal expressions in parentheses.\nDifferent operands between the terms of the expression: \"-\", \"+\"") << expr_6_1_2 << expr_6_1_1 << true;
